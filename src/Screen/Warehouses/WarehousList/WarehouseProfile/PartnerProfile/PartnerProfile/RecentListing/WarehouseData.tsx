...
interface ProfileListingProps {
  warehouseId: string; // Accept partnerId instead of partner
}
...
//   timestamp: string;
// }
const RecentListings: React.FC<ProfileListingProps> = ({ warehouseId }) => {
      ...
      try {
        do {
          const response = await apiService.get<{
            ...
            `/order/warehouse/order/history/${warehouseId}?page=${page}&limit=${usersPerPage}`
          );
    ...
    const fetchBankDetails = async () => {
      ...
      try {
        const response = await apiService.get<{
          ...
          errors: any;
          timestamp: string;
        }>(`/transaction/get/bank/detail/${partnerId}`);
          ...
          <thead className="text-xs text-gray-500 bg-gray-100">
            <tr>
              ...
              <th className="px-8 py-3 border-b border-gray-300">
                Payments (from User to BMW)
              </th>
              <th className="px-8 py-3 border-b border-gray-300">
                Payments (from BMW to partner)
              </th>
